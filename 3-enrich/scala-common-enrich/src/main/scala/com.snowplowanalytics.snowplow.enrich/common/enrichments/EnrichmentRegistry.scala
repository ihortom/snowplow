...
import registry.apilookup.{
  ApiLookupEnrichment,
  ApiLookupEnrichmentConfig
}
    ...
    enabled match {
      ...
      case e => {
        ...
        name.flatMap( nm => {
          ...
          } else if (nm == "api_lookup_enrichment_config") {
            ApiLookupEnrichmentConfig.parse(enrichmentConfig, schemaKey).map((nm, _).some)
          } else {
