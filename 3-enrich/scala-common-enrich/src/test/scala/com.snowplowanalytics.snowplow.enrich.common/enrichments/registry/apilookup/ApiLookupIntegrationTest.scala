...
object ApiLookupEnrichmentIntegrationTest {
...
import ApiLookupEnrichmentIntegrationTest._
class ApiLookupEnrichmentIntegrationTest extends Specification with ValidationMatchers { def is =
  "This is a integration test for the ApiLookupEnrichment" ^
    skipAllUnless(continuousIntegration) ^
    "Basic Case"                                       ! i1 ^
    "POST, Auth, JSON inputs, cache, several outputs"  ! i2 ^
                                                       end
  ...
  object IntegrationTests {
    ...
    val configuration2 = parseJson("""
        ...
        |           "field": "unstruct",
        |           "jsonPath": "$.jobflow_id",
        |           "schemaCriterion": "com.snowplowanalytics.monitoring.batch/emr_job_status/jsonschema/1-0-"
        |         }
        |				},
        |       {
        |         "key": "latitude",
        ...
        |           "field": "contexts",
        |           "jsonPath": "$.latitude",
        |           "schemaCriterion": "com.snowplowanalytics.snowplow/geolocation_context/jsonschema/1-1-0"
        |         }
        |				},
        |       {
        |         "key": "datetime",
        ...
        |           "jsonPath": "$.dt",
        |           "schemaCriterion": "org.openweathermap/weather/jsonschema/1-"
        |         }
        |       }
        |			],
        |			"api": {
  ...
  val SCHEMA_KEY = SchemaKey("com.snowplowanalytics.snowplow.enrichments", "api_lookup_enrichment_config", "jsonschema", "1-0-0")

  def i1 = {  // YES
    val config = ApiLookupEnrichmentConfig.parse(IntegrationTests.configuration, SCHEMA_KEY)
  ...
  def i2 = { // YES
    val config = ApiLookupEnrichmentConfig.parse(IntegrationTests.configuration2, SCHEMA_KEY)
    val event = new EnrichedEvent
    event.setApp_id("lookup-test")
    event.setUser_id("snowplower")
