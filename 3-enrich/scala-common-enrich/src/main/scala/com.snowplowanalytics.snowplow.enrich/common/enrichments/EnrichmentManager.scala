...
object EnrichmentManager {
  ...
  def enrichEvent(registry: EnrichmentRegistry, hostEtlVersion: String, etlTstamp: DateTime, raw: RawEvent)(implicit resolver: Resolver): ValidatedEnrichedEvent = {
    ...
    val apiLookupContexts = registry.getApiLookupEnrichment match {
      case Some(enrichment) => (customContexts, unstructEvent) match {
        case (Success(cctx), Success(ue)) =>
          val a = enrichment.lookup(event, preparedDerivedContexts, cctx, ue)
          a
    ...
    // Assemble prepared derived contexts with fetched via API Lookup
    val derived_contexts = apiLookupContexts.getOrElse(Nil) ++ preparedDerivedContexts
