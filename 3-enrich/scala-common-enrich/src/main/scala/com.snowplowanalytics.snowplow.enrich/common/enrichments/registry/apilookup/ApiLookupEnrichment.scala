...
object ApiLookupEnrichmentConfig extends ParseableEnrichment {
  ...
  def parse(config: JValue, schemaKey: SchemaKey): ValidatedNelMessage[ApiLookupEnrichment] = {
    isParseable(config, schemaKey).flatMap( conf => {
      ...
      } yield ApiLookupEnrichment(inputs, httpApi, outputs, cache)).toValidationNel
    })
  }
}

case class ApiLookupEnrichment(inputs: List[Input], api: HttpApi, outputs: List[Output], cache: Cache) extends Enrichment {
   ...
   *         outputs were failed
   */
  private[apilookup] def getOutputs(validInputs: Option[Map[String, String]]): Validation[String, List[JObject]] = {
    val result = for {
  ...
  /**
   * Check cache for URL and perform HTTP lookup if value wasn't found
   *
   ...
   */
  private[apilookup] def cachedOrRequest(url: String, output: Output): Validation[Throwable, JObject] = {
...
object ApiLookupEnrichment {
  ...
  private lazy val actorSystem = ActorSystem("api-lookup-system",
