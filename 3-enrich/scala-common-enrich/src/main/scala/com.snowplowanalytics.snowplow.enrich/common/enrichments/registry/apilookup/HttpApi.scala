...
case class HttpApi(method: String, uri: String, timeout: Int, authentication: Authentication) {
  ...
  private val authUser = for {
    httpBasic <- authentication.`http-basic`
   ...
   */
  private[apilookup] def buildUrl(context: Map[String, String]): Option[String] = {
...
object HttpApi {
   ...
   */
  private[apilookup] def everythingMatched(uri: String): Boolean =
   ...
   */
  private[apilookup] def replace(t: String, pair: (String, String)): String = pair match {
    case (key, value) =>
